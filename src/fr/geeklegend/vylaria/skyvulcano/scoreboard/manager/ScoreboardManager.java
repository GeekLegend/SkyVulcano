package fr.geeklegend.vylaria.skyvulcano.scoreboard.manager;
...
import fr.geeklegend.vylaria.skyvulcano.scoreboard.PersonalScoreboard;
...
public class ScoreboardManager
{
	...
	public ScoreboardManager()
	{
		...
		glowingTask = SkyVulcano.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
		{
			String ip = colorIpAt();
			for (PersonalScoreboard scoreboard : scoreboards.values())
				SkyVulcano.getInstance().getExecutorMonoThread().execute(() -> scoreboard.setLines(ip));
		}, 80, 80, TimeUnit.MILLISECONDS);

		reloadingTask = SkyVulcano.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
		{
			for (PersonalScoreboard scoreboard : scoreboards.values())
				SkyVulcano.getInstance().getExecutorMonoThread().execute(scoreboard::reloadData);
	...
	private String colorIpAt()
	{
		...
		formattedIp.append(ChatColor.GREEN).append(ip.charAt(ipCharIndex));
