package fr.geeklegend.vylaria.skyvulcano.scoreboard.objective;
...
import fr.geeklegend.vylaria.skyvulcano.scoreboard.reflection.Reflection;
...
public class VObjective
{
	...
	{
		this.receivers.stream().filter(OfflinePlayer::isOnline)
				.forEach(op -> RawObjective.updateObjective(op.getPlayer(), this));
	}
	...
	public static class RawObjective
	{
		...
		{
			Reflection.sendPacket(p, makeScoreboardObjectivePacket(0, objective.getName(), objective.getDisplayName(),
					objective.getFormat()));
		}

		public static void updateObjective(Player p, VObjective objective)
		{
			Reflection.sendPacket(p, makeScoreboardObjectivePacket(2, objective.getName(), objective.getDisplayName(),
					objective.getFormat()));
		}

		public static void removeObjective(Player p, VObjective objective)
		{
			Reflection.sendPacket(p, makeScoreboardObjectivePacket(1, objective.getName(), objective.getDisplayName(),
					objective.getFormat()));
		}

		public static void removeObjective(Player p, String name)
		{
			Reflection.sendPacket(p, makeScoreboardObjectivePacket(1, name, "",
		...
		{
			Reflection.sendPacket(p, makeScoreboardScorePacket(objective.getName(),
					PacketPlayOutScoreboardScore.EnumScoreboardAction.CHANGE, score.getPlayerName(), score.getScore()));
		}

		public static void updateScoreObjective(Player p, VObjective objective, VScore score, int scoreValue)
		{
			Reflection.sendPacket(p, makeScoreboardScorePacket(objective.getName(),
					PacketPlayOutScoreboardScore.EnumScoreboardAction.CHANGE, score.getPlayerName(), scoreValue));
		}
		...
		{
			Reflection.sendPacket(p, makeScoreboardScorePacket(objective.getName(),
					PacketPlayOutScoreboardScore.EnumScoreboardAction.REMOVE, score.getPlayerName(), 0));
		}

		public static PacketPlayOutScoreboardScore makeScoreboardScorePacket(String objectiveName, Object action,
		...
		public static PacketPlayOutScoreboardObjective makeScoreboardObjectivePacket(int action, String objectiveName,
		...
		public static PacketPlayOutScoreboardDisplayObjective makeScoreboardDisplayPacket(String objectiveName,
				int location)
		{
			try
			{
				...
				Reflection.setValue(packet, "a", location); // Emplacement de l'objective - 0 = list, 1 = sidebar, 2 =
															// belowName
				Reflection.setValue(packet, "b", objectiveName); // Nom de l'objective
